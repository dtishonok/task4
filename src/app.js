...
const pool = require('./db'); // убедись, что у тебя есть db.js с подключением к Postgres
...
app.get('/users', async (req, res) => {
  try {
    ...
    res.status(500).send('Server error while loading users');
  }
});
...
app.post('/bulk', async (req, res) => {
  try {
    ...
    if (!userIds) return res.redirect('/users');
    ...
    res.status(500).send('Server error while performing bulk action');
  }
});
